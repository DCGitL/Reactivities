...
export const useActivities = (id?: string | number) => {
  ...
  const {
    ...
    updateData,
    createData,
    deleteData } = useData<Activity>(activitiesEndPoint, id, currentUser);
  ...
  const updateAttendance = useMutation({
    ...
    onMutate: async (activityId: string) => {
      ...
      const prevActivity = queryClient.getQueryData<Activity>([activitiesEndPoint, activityId]);

      queryClient.setQueryData<Activity>([activitiesEndPoint, activityId], oldActivity => {
    ...
    onError: (error, activityId, context) =>{
      ...
      if(context?.prevActivity){
        queryClient.setQueryData([activitiesEndPoint, activityId], context.prevActivity)
      }

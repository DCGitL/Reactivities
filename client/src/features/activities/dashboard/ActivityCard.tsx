  ...
  return (
    <Card elevation={3} sx={{borderRadius: 3}}>
        ...
        <Box display='flex' alignItems='center' justifyContent={'space-between'}>
          <CardHeader
           avatar={<Avatar sx={{heith:40, width: 40}} />}
           ...
           subheader = {
            <>
               Hosteb by {' '} <Link to={`/profiles/${activity.hostId}`}>{activity.hostDisplayName}</Link>
            </>
           }
          />
        ...
        <CardContent sx={{padding: 0}}>
          <Box display={'flex'} alignItems={'center'} mb={2} paddingX={2}>
        ...
        <CardContent sx={{ pb:2}}>
           ...
           <Box sx={{display: 'flex', justifyContent: 'flex-end', borderRadius: 3}}>
             <Button size="medium"
                     component={Link} 
